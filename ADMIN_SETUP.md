...
6. **App Review → Permissions and Features**: اطلب Advanced Access للصلاحيات:
   ...
   - `business_management`
   ولميزة الرد التلقائي على الكومنتات (Comment-to-DM — صفحة `auto_replies.php`) هتحتاج كمان:
   - `instagram_manage_comments` (قراءة كومنتات الفيديو ومعرفة مين كتبها)
   - `instagram_manage_messages` (إرسال الرد الخاص والرسايل في الـ DM)
   - `pages_manage_metadata`
   لكل صلاحية هتحتاج تسجل **فيديو Screencast** يوضح بالظبط إزاي بتُستخدم في الموقع (رفع فيديو → نشر
   على انستجرام، وإنشاء قاعدة رد تلقائي → كومنت → استلام رسالة). ميتا بتاخد الفيديوهات دي بجدية —
   وضّح الخطوات كاملة وميتفوتكش أي جزء.
